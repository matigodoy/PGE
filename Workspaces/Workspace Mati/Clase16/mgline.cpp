...
MGLine::MGLine( QWidget * parent ) : QLineEdit( parent ),
...
void MGLine::setLista(const QStringList & newLista ) {
    lista = newLista;
}

void MGLine::sugerirCon(Sugerencia sugerencia)
{
...
/**
  ...
  */
bool MGLine::eventFilter( QObject * obj, QEvent * ev )  {
    if ( obj == popup )  {
...
/**
  ...
  */
void MGLine::slot_completarLineEdit()  {
    timer->stop();
    popup->hide();
    this->setFocus();
...
/**
  * Método para sugerir las sugerencias según lo que se vaya ingresando en el QLineEdit por el usuario.
  * Para hacer esto se consulta a la base de datos (sea un QStingList o Google) devolviendo un QVector< QStringList >.
  */
void MGLine::slot_sugerencia()   {
...
/**
  ...
  */
void MGLine::completarPopup( QVector< QStringList > vector )  {
    popup->clear();
...
void MGLine::slot_descargaFinalizada( QNetworkReply * reply )  {

    QString ba = reply->readAll();

    QStringList listaGoogle;

    // Parsear (o interpretar) el JSON para extraer cada una de las sugerencias de Google
