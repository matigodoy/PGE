...
public:
    ...
    bool pop_back();
    bool erase( int i );
    bool borrar_del_principio( int cuantos );
    bool borrar_del_final( int cuantos );
...
template < class T > bool Listado< T >::borrar_del_final( int cuantos )  {
...
template < class T > bool Listado< T >::borrar_del_principio( int cuantos )  {
    for( int i = 0 ; i < cuantos ; i++ )  {
        erase(0);
    }
    return true;
...
template < class T > void Listado< T >::operator+( int cuantasNuevasCeldas )  {
    Listado< T > * lAux = new Listado< T >( this->getMaxSize() + cuantasNuevasCeldas );
