...
FMLine::FMLine(QWidget * parent) : QLineEdit( parent ), cantidadMaximaDeRecomendaciones( 5 ), sugerencia(NADA), manager( new QNetworkAccessManager( this ) ) {
...
void FMLine::setLista(const QStringList & newLista ) {
    lista = newLista;
}

void FMLine::sugerirCon(Sugerencia sugerencia) {
...
/**
  ...
  */
bool FMLine::eventFilter( QObject * obj, QEvent * ev ) {
    if ( obj == popup )  {
...
/**
  ...
  */
void FMLine::slot_completarLineEdit()  {
    timer->stop();
    popup->hide();
    this->setFocus();
...
void FMLine::slot_descargaFinalizada( QNetworkReply * reply )  {
...
/**
  ...
  */
void FMLine::completarPopup( QVector< QStringList > vector )  {
    popup->clear();
